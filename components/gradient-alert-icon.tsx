import { useId } from "react"
import { cn } from "@/lib/utils"

type GradientAlertIconProps = React.ComponentPropsWithoutRef<"svg">

export function GradientAlertIcon({ className, ...props }: GradientAlertIconProps) {
  const id = useId()
  const gradId = `alert-grad-${id.replaceAll(":", "")}`

  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      aria-hidden="true"
      className={cn("size-5", className)}
      {...props}
    >
      <defs>
        <linearGradient id={gradId} x1="0" y1="0" x2="24" y2="24" gradientUnits="userSpaceOnUse">
          <stop offset="0%"   style={{ stopColor: "var(--primary-500)" }} />
          <stop offset="33%"  style={{ stopColor: "oklch(67.7% 0.131 242)" }} />
          <stop offset="66%"  style={{ stopColor: "oklch(66.9% 0.162 307)" }} />
          <stop offset="100%" style={{ stopColor: "var(--secondary-500)" }} />
        </linearGradient>
      </defs>
      {/* Alert02 icon — stroke paths with gradient, corrected to "!" layout */}
      <path
        d="M13.9248 21H10.0752C5.44476 21 3.12955 21 2.27636 19.4939C1.42317 17.9879 2.60736 15.9914 4.97574 11.9985L6.90057 8.75333C9.17559 4.91778 10.3131 3 12 3C13.6869 3 14.8244 4.91777 17.0994 8.75332L19.0243 11.9985C21.3926 15.9914 22.5768 17.9879 21.7236 19.4939C20.8704 21 18.5552 21 13.9248 21Z"
        stroke={`url(#${gradId})`}
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="1.5"
      />
      {/* Exclamation stem — top portion */}
      <path
        d="M12 9V14"
        stroke={`url(#${gradId})`}
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="1.5"
      />
      {/* Exclamation dot — bottom */}
      <circle
        cx="12"
        cy="17.25"
        r="0.75"
        fill={`url(#${gradId})`}
      />
    </svg>
  )
}
