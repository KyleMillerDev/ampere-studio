...
import { setOrderArchived } from "@/lib/stripe/orders"
import { archiveOrderSchema } from "@/lib/validation/order.schema"
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const parsed = archiveOrderSchema.safeParse(body)
  if (!parsed.success) {
  ...
  try {
    const order = await setOrderArchived(id, parsed.data.archived)
